    ...
    try {
      ...
      if (response.ok) {
        ...
        if (selectedConsultation?.id === id) {
           setSelectedConsultation({ ...selectedConsultation, status: "Confirmed" });
        }
      }
              ...
              {consultations.map((consult) => (
                <tr key={consult.id} className="hover:bg-cream/20 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center w-fit gap-1 ${consult.status.toLowerCase() === "confirmed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                      {consult.status.toLowerCase() === "confirmed" ? <FiCheckCircle size={12}/> : <FiXCircle size={12}/>}
                  ...
                  <td className="px-6 py-4 flex justify-end gap-3">
                    ...
                    <button onClick={() => handleConfirm(consult.id)} className="text-green-500 hover:text-green-700 p-2 hover:bg-green-50 rounded-lg" title="Confirm Booking"><FiCheckCircle size={18} /></button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-border/50">
              <div className="bg-gradient-to-r from-olive to-olive-dark p-6 text-white flex justify-between items-start">
                <div>
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center w-fit gap-1 bg-white/20 text-white shadow-sm backdrop-blur-md`}>
                    {selectedConsultation.status.toLowerCase() === "confirmed" ? <FiCheckCircle size={12}/> : <FiXCircle size={12}/>}
              ...
              <div className="p-4 border-t border-border bg-gray-50 flex justify-end gap-3">
                {selectedConsultation.status.toLowerCase() !== "confirmed" && (
                  <button onClick={() => { handleConfirm(selectedConsultation.id); setViewModalOpen(false); }} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg transition-colors flex items-center gap-2">
