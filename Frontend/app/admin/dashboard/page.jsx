...
export default function DashboardOverview() {
  ...
  // Safe Fetch Helper: Attaches Token & Guarantees an Array response
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [doctors, gallery, courses, packages, consultations] = await Promise.all([
          safeFetch(`${API_BASE_URL}/specialists/`, headers),
          safeFetch(`${API_BASE_URL}/gallery/`, headers),
          safeFetch(`${API_BASE_URL}/courses/`, headers),
          safeFetch(`${API_BASE_URL}/packages/`, headers),
          safeFetch(`${API_BASE_URL}/consultations/`, headers)
        ]);

        setRawData({ doctors, gallery, courses, packages, consultations });
  ...
  const getFilteredConsultations = () => {
    ...
    // Fallback safeguard to prevent .filter crash
    if (!Array.isArray(rawData.consultations)) return [];
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
              ...
              <div className="relative" data-html2canvas-ignore>
                <div className="flex bg-white border border-border rounded-lg shadow-sm cursor-pointer" onClick={() => setShowDropdown(!showDropdown)}>
                  ...
                  <button disabled={isDownloading} className="px-3 py-2 flex items-center hover:bg-gray-50 transition-colors">
                ...
                <AnimatePresence>
                  {showDropdown && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-border overflow-hidden z-10">
                      {["Daily", "Weekly", "Monthly"].map((type) => (
                        <button key={type} onClick={() => { setReportType(type); setShowDropdown(false); }} className={`w-full text-left px-4 py-3 text-sm transition-colors ${reportType === type ? "bg-olive/10 text-[#6b7c5b] font-semibold" : "hover:bg-gray-50 text-text"}`}>
                          {type} Report
                        </button>
                      ))}
                      <div className="border-t border-border p-2">
                        <button onClick={(e) => { e.stopPropagation(); handleDownloadPDF(); }} disabled={isDownloading} className="w-full bg-[#6b7c5b] text-white py-2 rounded-md text-sm font-medium hover:bg-[#4a5740] transition-colors flex justify-center items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {dynamicStats.map((stat, index) => {
                ...
                return (
                  <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="bg-white rounded-xl p-6 shadow-sm border border-border flex items-center gap-4 hover:shadow-md transition-shadow">
            ...
            <div className="w-full mb-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="bg-white rounded-xl p-6 shadow-sm border border-border">
